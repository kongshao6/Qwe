...
local function showSuccessNotifications()
    ...
    notifyGui.Name = "SuccessNotify"
    notifyGui.ResetOnSpawn = false
    notifyGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    notifyGui.Parent = playerGui
    
    local notifications = {
        {Text = "✅ 卡密验证成功", Color = Color3.fromRGB(0, 200, 0), Icon = "✅"},
        {Text = "🔓 正在加载脚本...", Color = Color3.fromRGB(255, 165, 0), Icon = "🔓"},
        {Text = "⚡ KS Script 启动中", Color = Color3.fromRGB(255, 60, 60), Icon = "⚡"},
    }
    ...
    for i, notif in ipairs(notifications) do
        ...
        local notifStroke = createStroke(notifFrame, 2, notif.Color)
        
        local iconLabel = Instance.new("TextLabel")
        iconLabel.Size = UDim2.new(0, 40, 1, 0)
        iconLabel.Position = UDim2.new(0, 5, 0, 0)
        iconLabel.Text = notif.Icon
        ...
        notifFrame.Position = UDim2.new(0, -260, 0, 10 + (i - 1) * 60)
    ...
    for i = #notifications, 1, -1 do
        local notifFrame = notifyGui:GetChildren()[i]
        if notifFrame and notifFrame:IsA("Frame") then
            local slideOut = TweenService:Create(notifFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
                Position = UDim2.new(0, -260, 0, 10 + (i - 1) * 60)
            })
            slideOut:Play()
            task.wait(0.2)
        end
...
function loadMainScript()
    ...
    moveGroup:AddToggle('MyWalkToggle', {
        Text = '自定义速度',
        Default = false,
        Tooltip = '开启后可以调整走路速度',
    ...
    moveGroup:AddSlider('MyWalkSpeed', {
        Text = '速度值',
        Default = 50,
        Min = 16,
        Max = 200,
    ...
    moveGroup:AddToggle('MyJumpToggle', {
        Text = '自定义跳跃',
        Default = false,
        Tooltip = '开启后可以调整跳跃高度',
        Callback = function(Value)
    ...
    moveGroup:AddSlider('MyJumpPower', {
        Text = '跳跃值',
        Default = 100,
        Min = 50,
        Max = 300,
    ...
    moveGroup:AddToggle('MyGravityToggle', {
        Text = '自定义重力',
        Default = false,
        Callback = function(Value)
    ...
    moveGroup:AddSlider('MySpinSpeed', {
        Text = '旋转速度',
        Default = 10,
        Min = 1,
        Max = 100,
    ...
    moveGroup:AddToggle('MyInfiniteJump', {
        Text = '无限跳',
        Default = false,
        Callback = function(Value)
    ...
    visualGroup:AddToggle('MyFovToggle', {
        Text = '自定义视野',
        Default = false,
        Callback = function(Value)
            fovEnabled = Value
    ...
    visualGroup:AddToggle('MyNightVision', {
        Text = '夜视模式',
        Default = false,
        Callback = function(Value)
    ...
    visualGroup:AddToggle('MyFullBright', {
        Text = '全亮模式',
        Default = false,
        Callback = function(Value)
